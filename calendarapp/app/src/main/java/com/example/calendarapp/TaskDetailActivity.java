...
public class TaskDetailActivity extends AppCompatActivity {
    ...
    private Map<String, String> parseWeatherDataByDate(List<WeatherService.WeatherResponse.WeatherElement> elements) {
        Map<String, String> dailyWeatherInfo = new LinkedHashMap<>();
            ...
            for (WeatherService.WeatherResponse.WeatherElement element : elements) {
                for (WeatherService.WeatherResponse.TimeData timeData : element.getTime()) {
                    if (timeData != null && timeData.getElementValues() != null &&
                        ...
                        String date = timeData.getStartTime().split("T")[0]; // 獲取日期部分

                        // 初始化每天的天氣資訊
                        dailyWeatherInfo.putIfAbsent(date, "");

                        switch (element.getElementName()) {
                            case "最高溫度":
                            case "MaxT":
                                dailyWeatherInfo.put(date, dailyWeatherInfo.get(date) +
                                        String.format("最高溫度: %s°C\n", value));
                                break;
                            case "最低溫度":
                            case "MinT":
                                dailyWeatherInfo.put(date, dailyWeatherInfo.get(date) +
                                        String.format("最低溫度: %s°C\n", value));
                                break;
                            case "平均溫度":
                                dailyWeatherInfo.put(date, dailyWeatherInfo.get(date) +
                                        String.format("平均溫度: %s°C\n", value));
                                break;
                        }
                    }
                }
